...
export default function Home() {
  ...
  const loadRound = useCallback(async () => {
    ...
    if (!roundData) {
      const { data: lastRound } = await supabase
        ...
        .single()
      if (lastRound) {
        await enrichAndSetRound(lastRound, true)
      }
      setPageLoading(false)
      return
    }

    await enrichAndSetRound(roundData, false)
    setPageLoading(false)
  }, [])
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      setError(err.message?.includes('unique')
        ? 'That email is already registered under a different name.'
  ...
  const optStyle = (selected, isCorrect, isWrong) => ({
    ...
    opacity: (locked || (hasSaved && !editing)) && !selected ? 0.45 : 1,
  })
        ...
        {step === 'register' && (
          <div className="card" style={{ maxWidth: 460 }}>
            ...
            <p style={{ color: 'var(--text2)', fontSize: 14, marginBottom: 24 }}>
              Enter your name and email. Returning? Use the same email to pick up where you left off.
            </p>
            <form onSubmit={handleRegister}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 6 }}>Used only to identify you across rounds. No spam.</p>
              </div>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
              <div>
                {pageLoading ? (
                  ...
                  <>
                    <h2 style={{ fontSize: 24, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <div>
                    <h2 style={{ fontSize: 22, color: 'var(--text2)' }}>No active round</h2>
                    <p style={{ fontSize: 14, color: 'var(--text3)', marginTop: 6 }}>The admin hasn't posted today's pairings yet.</p>
                  </div>
                )}
              </div>
              <div style={{ textAlign: 'right' }}>
                ...
                <button onClick={() => { sessionStorage.clear(); setStep('register'); setParticipant(null); setPredictions({}); setSavedPreds({}); setEditing(false); }}
            ...
            {editing && !locked && (
              <div style={{ marginBottom: 20, padding: '12px 18px', borderRadius: 8, background: 'rgba(224,85,85,0.08)', border: '1px solid rgba(224,85,85,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                ...
                <button onClick={cancelEdit} style={{ padding: '6px 14px', borderRadius: 8, background: 'none', border: '1px solid rgba(224,85,85,0.4)', color: 'var(--red)', fontFamily: 'DM Sans', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
              </div>
            )}
                ...
                <div key={game.id} className="card" style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                    {opts.map(opt => {
                      ...
                      return (
                        <button key={opt.value} onClick={() => pick(game.id, opt.value)} style={optStyle(selected, isCorrect, isWrong)}>
                          ...
                          {editing && wasSelected && !selected && <p style={{ fontSize: 10, marginTop: 4, color: 'var(--text3)' }}>was saved</p>}
                          {editing && !wasSelected && selected && <p style={{ fontSize: 10, marginTop: 4, color: 'var(--gold)' }}>new pick</p>}
                        </button>
                      )
                    })}
                  </div>
            ...
            {round && !locked && games.length > 0 && (
              <div style={{ marginTop: 8 }}>
                ...
                {!hasSaved && (
                  <button onClick={handleSubmit} disabled={loading || !allPicked} className="btn-gold"
